...
当前范围已纳入预约申请与人工人员安排，不包含支付退款、复杂容量与自动派单、投诉回访与统计、独立审计与隐私申请模块。档案和预约自身的变更记录与访问保护继续保留。
...
履约已接入预约任务，开始、完成、异常和终止共用预约状态及事件，不另建一套履约主状态。服务目录与授权服务复用预约模块的持久化对象，事务在对应 Service，复杂对象权限和时间冲突查询使用 MyBatis XML。
...
- 当前小程序采用服务器会话与 CSRF，Cookie 只在运行内存保存；短期访问令牌与刷新令牌轮换仍为后续规划。
